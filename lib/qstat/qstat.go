...
import (
	"fmt"
	"github.com/paratera/pbspro_exporter/lib/utils"
	"strconv"
	"strings"
	"unsafe"
...
//查询指定作业的信息
func (qs *Qstat) Pbs_statjob() ([]utils.BatchStatusGo, error) {
	i := C.CString(qs.ID)
...
func (qs *Qstat) PbsJobsState() error {
	...
	for _, jobs := range batch {
		...
		for _, attr := range jobs.Attributes {
			...
			default:
				fmt.Println("other jobs attr", attr)
			}
		}
...
//获取信息
func get_pbs_batch_status(batch_status *C.struct_batch_status) (batch []utils.BatchStatusGo) {
	...
	for batch_status != nil {
		...
		batch = append(batch, utils.BatchStatusGo{
